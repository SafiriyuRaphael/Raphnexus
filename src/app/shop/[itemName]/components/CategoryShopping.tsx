...
import CategoryFilterShop from "./CategoryFilterShop";
...
export default function CategoryShopping({ menu }: MenuProps) {
  const [listView, setListView] = useState(false);
  const [newMenu,setNewMenu]=useState<MenuData | null>(menu)
  ...
  const sortedItems = newMenu.categories
  ...
  return (
    <section className="px-7 py-16 flex gap-8">
      <div className="flex flex-col gap-4" onClick={() => setSideFilter(false)}>
          ...
          <div className="flex">
            <div className="flex gap-2 sm:w-full w-[50vw] justify-end sm:gap-3 items-center">
              ...
              <div className="relative">
                ...
                {isDropdownOpen && (
                  <ul className="absolute bg-[#FBF7E8] border w-full border-black shadow-md mt-1 z-[55] text-sm md:text-md  text-gray-700">
                    <li
              ...
              >
                ...
                <div
                  className={`bg-amber-100  w-full rounded-3xl group-hover:h-full group-hover:bg-amber-400 transition-all transform duration-500 ease-in-out ${
                    !listView ? "h-24 sm:h-36 md:h-16 " : "h-44"
                  }`}
                ></div>
              </div>
              {!listView && (
